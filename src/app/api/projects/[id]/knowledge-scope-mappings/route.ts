...
import {
  createKnowledgeScopeMappingForProject,
  listKnowledgeScopeMappings,
...
function mapScopeMappingErrorToStatus(error: unknown): number {
  ...
  if (error.message.includes('暂时无法解析') || error.message.includes('required')) return 400
  return 500
}
...
async function createFromJson(projectId: string, request: Request) {
  ...
  const data = createKnowledgeScopeMappingForProject(projectId, {
    name: body.name,
...
async function createFromFormData(projectId: string, request: Request) {
  ...
  for (const file of files) {
    ...
    data.push(
      createKnowledgeScopeMappingForProject(projectId, {
...
export async function GET(
  ...
  try {
    ...
    const data = listKnowledgeScopeMappings(id)
    return NextResponse.json({ success: true, data, error: null })
  } catch (error) {
    console.error('[GET /api/projects/[id]/knowledge-scope-mappings]', error)
    return NextResponse.json(
      { success: false, data: null, error: 'Failed to fetch knowledge scope mappings' },
      { status: 500 },
    )
  }
}
...
export async function POST(
  ...
  try {
    ...
    console.error('[POST /api/projects/[id]/knowledge-scope-mappings]', error)
    return NextResponse.json(
      {
        success: false,
        data: null,
        error: error instanceof Error ? error.message : 'Failed to create knowledge scope mapping',
      },
