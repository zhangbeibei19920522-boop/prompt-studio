...
\`\`\`
- 记忆指令（当用户要求记住/忘记某些内容时使用。先确认操作，再附上 JSON）：
\`\`\`json
{"type":"memory","data":{"command":"create","scope":"project","content":"记忆内容"}}
\`\`\`
- 删除记忆：
\`\`\`json
{"type":"memory","data":{"command":"delete","scope":"project","memoryId":"记忆ID"}}
\`\`\`
- 列出记忆：
\`\`\`json
{"type":"memory","data":{"command":"list","scope":"project"}}
\`\`\`
- 普通对话直接用文字回复，不需要 JSON 格式

## 记忆系统

你具备跨会话记忆能力，可以记住用户的偏好和业务知识。

### 优先级规则
- **项目记忆** 优先级高于 **全局记忆**
- 当两者冲突时，以项目记忆为准，静默覆盖，不需要询问用户

### 记忆指令识别
用户可能通过自然语言触发记忆操作：
- **"记住 XXX"** / **"以后都这样"** → 创建项目记忆（scope: project）
- **"所有项目都记住 XXX"** / **"全局记住 XXX"** → 创建全局记忆（scope: global）
- **"忘掉 XXX"** / **"删除记忆 XXX"** → 删除匹配的记忆（command: delete）
- **"我的记忆有哪些"** / **"列出记忆"** → 列出记忆（command: list）

### 记忆分类
- **preference** — 用户偏好（如"用中文写 prompt"、"偏好简洁风格"）
- **fact** — 业务事实（如"公司名称为 XX"、"目标用户为北美市场"）

创建记忆时，你需要判断内容属于哪个分类，并在 content 中用简洁明确的语言描述。`
...
export function buildPlanMessages(context: AgentContext): ChatMessage[] {
  ...
  if (context.globalMemories.length > 0) {
    system += '\n\n## 全局记忆（通用偏好和知识）'
    for (const m of context.globalMemories) {
      system += `\n- [${m.category}] ${m.content} (ID: ${m.id})`
    }
  }

  // Project memories (higher priority than global)
  if (context.projectMemories.length > 0) {
    system += '\n\n## 项目记忆（本项目专属，优先级高于全局）'
    for (const m of context.projectMemories) {
      system += `\n- [${m.category}] ${m.content} (ID: ${m.id})`
    }
  }
