...
export function DetailView({
  ...
  const copy = useMemo(() => stateCopy(detailState), [detailState])
  const detailTitle =
    detailMode === "prod"
      ? "当前 PROD 版本"
      : detailMode === "stg"
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <MetricCard label="当前轮次" value="第 2 轮" helper="本轮内容草稿可查看" tone="blue" size="mini" />
          <MetricCard
            label="待确认"
...
type RiskTab = "summary" | "conflict" | "delete"
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5">
      ...
      <div className="mt-5 space-y-4">
        <ConflictRiskCard
          item={conflictRiskItems[0]}
          resolvedResult={resolvedResults[conflictRiskItems[0].id]}
          editableContent={editableContents[conflictRiskItems[0].id]}
          locked={locked}
          compact
          onResolve={onResolve}
        ...
        />
        <DeleteRiskCard
          item={deleteRiskItems[0]}
          resolvedResult={resolvedResults[deleteRiskItems[0].id]}
          editableContent={editableContents[deleteRiskItems[0].id]}
          locked={locked}
          compact
          onResolve={onResolve}
          onUpdateEditableContent={onUpdateEditableContent}
  ...
  return (
    <div className="rounded-lg border border-rose-100 bg-white p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <RiskTextBox title="系统建议保留的内容" content={currentRoundContent} accent="add" />
      </div>
                    ...
                    <section className="mt-4 rounded-lg border border-slate-200 bg-white p-4">
                      ...
                      <div className="mt-4 space-y-4 text-sm">
                        ...
                        <div className="border-t border-slate-100 pt-4">
                          ...
                          <div className="mt-2 space-y-2 leading-6 text-slate-700">
                            {selectedRound.orphanRecords.length > 0 ? (
                              selectedRound.orphanRecords.map((item) => <p key={item}>{item}</p>)
                            ) : (
                              <p>当前没有需要补充归类的内容。</p>
                            )}
                          </div>
                        ...
                        <div className="border-t border-slate-100 pt-4">
                          ...
                          <div className="mt-2 space-y-2 leading-6 text-slate-700">
                            {selectedRound.ambiguityRecords.length > 0 ? (
                              selectedRound.ambiguityRecords.map((item) => <p key={item}>{item}</p>)
                            ) : (
                        ...
                        <div className="border-t border-slate-100 pt-4">
                          ...
                          <div className="mt-2 space-y-2 leading-6 text-slate-700">
                            {selectedRound.missingRecords.map((item) => (
                              <p key={item}>{item}</p>
                            ))}
                          </div>
                        </div>
                    ...
                    >
                      ...
                      <div className="mt-4 space-y-4 text-sm">
                        <div>
                          ...
                          <div className="mt-2 space-y-2 leading-6 text-slate-700">
                            {selectedIndexVersion.coverageAudit.reasons.map((reason) => (
                        ...
                        <div className="border-t border-slate-100 pt-4">
                          ...
                          <div className="mt-2 space-y-2 leading-6 text-slate-700">
                            {selectedIndexVersion.coverageAudit.orphanRecords.length > 0 ? (
                              selectedIndexVersion.coverageAudit.orphanRecords.map((item) => <p key={item}>{item}</p>)
                            ) : (
                              <p>当前没有待归类内容。</p>
                            )}
                          </div>
                        ...
                        <div className="border-t border-slate-100 pt-4">
                          ...
                          <div className="mt-2 space-y-2 leading-6 text-slate-700">
                            {selectedIndexVersion.coverageAudit.ambiguityRecords.length > 0 ? (
                              selectedIndexVersion.coverageAudit.ambiguityRecords.map((item) => <p key={item}>{item}</p>)
                            ) : (
                              <p>当前没有可能重复的内容。</p>
                            )}
                          </div>
